...
이것들은
[results/models.py](../../src/scribe/results/models.py)의
...
### `recovered_to_outbox`와 `replay_refs`

이 두 필드는 degraded 결과가 단지 경고 수준인지, 아니면 실시간 전달 실패 뒤에도 durable
recovery 경로가 확보된 상태인지를 구분할 때 중요합니다.

- `recovered_to_outbox=True`이면 payload가 outbox에 안전하게 남았습니다.
- `replay_refs`는 나중에 replay나 운영 추적에서 사용할 수 있는 reference 집합입니다.
...
[runtime/dispatch.py](../../src/scribe/runtime/dispatch.py)의
...
### 4. 실시간 Sink는 모두 실패했지만 Outbox에는 남았다

이 경우는 complete loss와 같은 뜻이 아닙니다.

- 실시간 delivery는 실패했고,
- payload는 durable outbox에 남았고,
- 결과는 `failure` 대신 `degraded`로 귀결될 수 있습니다.

즉 현재 구현에서 degraded는 "실시간 목표 delivery는 놓쳤지만 truth는 나중에 복구 가능하게 남았다"는 뜻까지 포함할 수 있습니다.
...
[artifacts/service.py](../../src/scribe/artifacts/service.py)의
...
1. [싱크와 저장소](sinks-and-storage.md):
