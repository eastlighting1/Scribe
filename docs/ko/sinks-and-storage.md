...
핵심 abstract type은 [Sink](../../src/scribe/sinks/base.py)
입니다.
...
이 helper는 모두
[runtime/dispatch.py](../../src/scribe/runtime/dispatch.py)에
...
이 다섯 가지는 서로 중복되지 않습니다. 각각 다른 사용 상황을 위한 도구입니다.
...
구현은
[sinks/memory.py](../../src/scribe/sinks/memory.py)에 있습니다.
...
구현은
[sinks/composite.py](../../src/scribe/sinks/composite.py)에
있습니다.
...
없애주지는 않는다는 것입니다. fan-out behavior를 묶어 줄 뿐입니다. 새 통합에서는
top-level `Scribe(..., sinks=[...])` fan-out이 권장됩니다.

## `S3ObjectSink`

`S3ObjectSink`는 append형 로컬 JSONL 대신, payload 하나를 object 하나로 써야 하는
원격 object storage 환경에 맞춘 sink입니다.

구현은
[adapters/s3/json_objects.py](../../src/scribe/adapters/s3/json_objects.py)
에 있습니다.

이 sink는 다음 상황에 잘 맞습니다:

- append race를 피하고 싶은 경우,
- object-store 기반 수집이 필요한 경우,
- family/date/ref 기반 key로 later replay나 inspection을 하고 싶은 경우.

## `KafkaSink`

`KafkaSink`는 family별 topic으로 payload를 전달하는 streaming-oriented sink입니다.

구현은
[adapters/kafka/producer.py](../../src/scribe/adapters/kafka/producer.py)
에 있습니다.

이 sink는 producer ack를 동기적으로 확인하므로, dispatch retry, durable outbox, replay와
같은 운영 recovery path와 자연스럽게 연결됩니다.
...
- 실시간 eligible sink가 모두 실패해도 durable outbox 저장에 성공하면 결과는 대개 `degraded`가 되며,
- eligible sink가 모두 실패하고 outbox recovery도 없으면 `Scribe`는 `SinkDispatchError`를 raise합니다.
